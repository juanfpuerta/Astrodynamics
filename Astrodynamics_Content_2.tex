    ...
    \begin{columns}[t]
        \begin{column}{0.52\textwidth}
            \begin{block}{Laplace Sphere of Influence ($r_{\text{SOI}}$)}
                \scriptsize
                Defines the boundary where the planetocentric 2-body approximation is more accurate than the heliocentric model[cite: 2]:
                \vspace{-0.1cm}
                \begin{equation*}
                ...
                \end{equation*}
                \vspace{-0.15cm}
                where $d$ is the planetary orbital distance from the Sun[cite: 2].
            \end{block}
            ...
            \begin{block}{Hill Sphere ($r_H$)}
                \scriptsize
                Defines the outer limit for stable closed orbits under rotating 3-body tidal forces[cite: 2]:
                \vspace{-0.1cm}
                \begin{equation*}
        ...
        \begin{column}{0.45\textwidth}
            \begin{block}{SOI Radius Comparison}
                ...
                \end{tabular}
                \par\vspace{1pt}
                \tiny $^*$Moon distance relative to Earth center[cite: 2].
            \end{block}
            ...
            \begin{alertblock}{Patched Conics Boundary}
                \scriptsize
                Patched conics assumes an instantaneous switch between 2-body regimes at $r = r_{\text{SOI}}$[cite: 2].
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
        ...
        \begin{column}{0.47\textwidth}
            ...
            \begin{exampleblock}{Physical Consequence}
                ...
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item Fast inner planets (Mercury) endure strong solar tides ($n_p \uparrow$), shrinking their SOI[cite: 2].
                    \item Slow outer planets (Jupiter) have weak solar tides ($n_p \downarrow$), allowing enormous SOI volumes[cite: 2].
                \end{itemize}
            \end{exampleblock}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            \begin{block}{Bodies With Natural Moons}
                \scriptsize
                For Earth, Mars, or Jupiter, optical tracking of natural satellites yields $\mu_p$ via Kepler's 3rd Law[cite: 2]:
                \vspace{-0.12cm}
                \begin{equation*}
                    \mu_p \approx \frac{4\pi^2 a_{\text{moon}}^3}{T_{\text{moon}}^2}
                \end{equation*}
                \vspace{-0.2cm}
                Yields high accuracy without in-situ missions[cite: 2].
            \end{block}
            ...
            \begin{alertblock}{The ``No-Moon'' Dilemma}
                \scriptsize
                \textbf{Mercury, Venus, and asteroids lack moons!}[cite: 2]
                \begin{itemize}\setlength{\itemsep}{0pt}\setlength{\topsep}{1pt}
                    \item Pre-spaceflight mass estimates relied on weak perturbations ($\pm 1\%\text{--}5\%$ error)[cite: 2].
                    \item Internal composition remained uncertain[cite: 2].
                \end{itemize}
            \end{alertblock}
        \end{column}
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Artificial Satellite Orbit Determination}
                \scriptsize
                Placing an artificial probe in a closed orbit[cite: 2]:
                \begin{enumerate}\setlength{\itemsep}{1pt}\setlength{\topsep}{1pt}
                    \item Radio Doppler tracking accurately measures orbit period $T_{\text{sc}}$ and semi-major axis $a_{\text{sc}}$[cite: 2].
                    \item Invert Kepler's Third Law[cite: 2]:
                    \vspace{-0.12cm}
                    \begin{equation*}
                        \mu_p = \frac{4\pi^2 a_{\text{sc}}^3}{T_{\text{sc}}^2}
                    \end{equation*}
                    \vspace{-0.2cm}
                    \item Line-of-sight range-rate residuals map higher geopotential harmonics ($J_2, C_{nm}, S_{nm}$)[cite: 2].
                \end{enumerate}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Worked Example: Measuring the Mass of Moonless Venus}
    \begin{exampleblock}{Spacecraft Orbit Benchmark (e.g., Magellan / Venus Express)}
        \scriptsize
        A science probe is placed into a circular orbit around Venus ($R_V = 6051.8\text{ km}$)[cite: 2]. DSN Doppler tracking measures an orbital altitude $h = 500.0\text{ km}$ and period $T = 5621.5\text{ s}$[cite: 2]. \\
        Given $G = 6.67430 \times 10^{-20}\text{ km}^3/\text{kg}\cdot\text{s}^2$, $d_V = 1.0816 \times 10^8\text{ km}$, $\mu_\odot = 1.32712 \times 10^{11}\text{ km}^3/\text{s}^2$[cite: 2]:
    \end{exampleblock}
    ...
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            ...
            \par\vspace{1pt}
            $a = R_V + h = 6051.8 + 500.0 = \mathbf{6551.8}\text{ km}$[cite: 2]

            \vspace{0.08cm}
            \textbf{Step 2: Gravitational Parameter ($\mu_V$)}
            \par\vspace{1pt}
            $\mu_V = \frac{4\pi^2 a^3}{T^2} = \frac{4\pi^2 (6551.8)^3}{(5621.5)^2} = \mathbf{324858.6}\text{ km}^3/\text{s}^2$[cite: 2]

            \vspace{0.08cm}
            \textbf{Step 3: Planetary Mass ($M_V$)}
            \par\vspace{1pt}
            $M_V = \frac{\mu_V}{G} = \frac{324858.6}{6.67430 \times 10^{-20}} = \mathbf{4.8673} \times 10^{24}\text{ kg}$[cite: 2]
        \end{column}
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{align*}
                ...
                &= \mathbf{6.165} \times 10^5\text{ km} \approx \mathbf{101.9}\,R_V \quad \text{[cite: 2]}
            \end{align*}

            \vspace{-0.2cm}
            \begin{alertblock}{Precision Outcome}
                \scriptsize
                Direct Doppler tracking resolved $\mu_V$ to 7 significant figures, reducing mass uncertainty from $\sim 2\%$ down to $< 0.001\%$[cite: 2].
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[t]
        \begin{column}{0.54\textwidth}
            \begin{block}{Heliocentric $\to$ Planetocentric Patching}
                \scriptsize
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item Heliocentric 3D Lambert transfer yields $\mathbf{v}_1, \mathbf{v}_2$[cite: 2].
                    \item \textbf{Hyperbolic Excess Velocity} vector at departure[cite: 2]:
                    \vspace{-0.05cm}
                    \begin{equation*}
                        \mathbf{v}_{\infty,\text{dep}} = \mathbf{v}_{\text{Lambert},1} - \mathbf{v}_{\text{planet},1} \quad \text{[cite: 2]}
                    \end{equation*}
                    \vspace{-0.2cm}
                    \item Departure parameters: $C_3 = v_\infty^2$, $a_h = -\mu_p / v_\infty^2$, $e_h = 1 + r_p v_\infty^2 / \mu_p$[cite: 2].
...
\begin{frame}{The Patched Conic Section Method}
    ...
    \IfFileExists{Images/IT_1.jpg}{
        \includegraphics[width=0.88\textwidth,height=0.74\textheight,keepaspectratio]{Images/IT_1.jpg}
    }{
        \begin{tikzpicture}
    ...
    \begin{columns}[t]
        \begin{column}{0.54\textwidth}
            \begin{block}{Approximation of the $N$-Body Problem}
                \scriptsize
                The gravitational $N$-body problem is non-integrable. The \textbf{Patched Conic Method} approximates the trajectory as three sequential 2-body Keplerian regimes[cite: 2]:
                \begin{enumerate}\setlength{\itemsep}{2pt}\setlength{\topsep}{1pt}
                    \item \textbf{Phase 1: Planetocentric Departure ($r < r_{\text{SOI},1}$)} \\
                    Central body: Earth ($\mu_1$). Hyperbolic escape from circular parking orbit[cite: 2].
                    \item \textbf{Phase 2: Heliocentric Transfer ($r_{\text{SOI},1} \le R \le r_{\text{SOI},2}$)} \\
                    Central body: Sun ($\mu_\odot$). Keplerian transfer ellipse connecting departure and arrival planetary orbits[cite: 2].
                    \item \textbf{Phase 3: Planetocentric Arrival ($r < r_{\text{SOI},2}$)} \\
                    Central body: Target planet ($\mu_2$). Hyperbolic encounter, capture burn, or planetary flyby[cite: 2].
                \end{enumerate}
            \end{block}
        \end{column}
        ...
        \begin{column}{0.43\textwidth}
            \begin{block}{Patch Boundary Rule}
                ...
                \vspace{-0.08cm}
                \begin{equation*}
                    \mathbf{V}_{\text{heliocentric}} = \mathbf{V}_{\text{planet}} + \mathbf{v}_{\infty} \quad \text{[cite: 2]}
                \end{equation*}
            \end{block}
            ...
            \begin{alertblock}{Energy Conservation Across Frames}
                \scriptsize
                Inside the SOI, planetocentric specific energy is conserved[cite: 2]:
                \vspace{-0.08cm}
                \begin{equation*}
                    C_3 = v_\infty^2 = v_p^2 - \frac{2\mu_p}{r_p} = \text{const} \quad \text{[cite: 2]}
                \end{equation*}
            \end{alertblock}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[t]
        \begin{column}{0.50\textwidth}
            \begin{block}{Geocentric Escape Mechanics}
                \scriptsize
                \begin{enumerate}\setlength{\itemsep}{2pt}\setlength{\topsep}{1pt}
                    \item Spacecraft waits in circular parking orbit ($r_p = R_1 + h_p$) at orbital speed $v_c = \sqrt{\mu_1 / r_p}$[cite: 2].
                    \item Trans-Planetary Injection (TMI) burn $\Delta v_{\text{inj}}$ is executed tangentially at periapsis[cite: 2]:
                    \vspace{-0.08cm}
                    \begin{equation*}
                        v_p = \sqrt{v_{\infty,\text{dep}}^2 + \frac{2\mu_1}{r_p}} \implies \Delta v_{\text{inj}} = v_p - v_c \quad \text{[cite: 2]}
                    \end{equation*}
                    \vspace{-0.15cm}
                    \item Spacecraft follows a hyperbolic escape arc, crossing Earth's SOI with excess speed $\mathbf{v}_{\infty,\text{dep}}$[cite: 2].
                \end{enumerate}
            \end{block}
            ...
            \begin{exampleblock}{Characteristic Launch Energy ($C_3$)}
                \scriptsize
                $C_3 = v_{\infty,\text{dep}}^2 = 2\mathcal{E}_h$ measures launch vehicle escape performance ($C_3 > 0$ for interplanetary escape)[cite: 2].
            \end{exampleblock}
        \end{column}
    ...
    \begin{columns}[t]
        \begin{column}{0.52\textwidth}
            ...
            \begin{block}{2. True Anomaly in the Parking Orbit}
                ...
                \end{equation*}
                \vspace{-0.15cm}
                The spacecraft must coast in its circular parking orbit until it is positioned exactly \textbf{$\beta$ degrees away} from the target asymptote direction before firing!
            \end{block}
        \end{column}
    ...
    \begin{columns}[t]
        \begin{column}{0.50\textwidth}
            \begin{block}{Definition of Characteristic Energy ($C_3$)}
                \scriptsize
                When mission planners design interplanetary trajectories, they specify the \textbf{Characteristic Energy ($C_3$)}:
                \vspace{-0.08cm}
                \begin{equation*}
            ...
            \begin{block}{Physical Meaning}
                \scriptsize
                $C_3$ measures the excess specific energy available to escape Earth's gravity and cruise to the destination:
                \begin{itemize}\setlength{\itemsep}{1.5pt}
                    \item $\mathbf{C_3 = 0}$: Spacecraft barely reaches parabolic escape ($v_\infty = 0$), entering an orbit identical to Earth's.
                    \item $\mathbf{C_3 > 0}$: Spacecraft escapes with residual speed $v_\infty$ to climb outward to Mars or dive inward to Venus.
                \end{itemize}
            \end{block}
        \end{column}
        ...
        \begin{column}{0.47\textwidth}
            \begin{block}{Why Use $C_3$ Instead of $\Delta V$?}
                ...
                \begin{enumerate}\setlength{\itemsep}{2pt}
                    ...
                    $\Delta V$ depends heavily on the chosen parking orbit altitude $r_p$. $C_3$ is an absolute energy state independent of parking orbit altitude!
                    \item \textbf{Dictates Launch Vehicle Selection:} \\
                    Launch providers (NASA, SpaceX, ULA) publish standardized payload performance curves:
                    \vspace{-0.05cm}
                    \begin{equation*}
                        \text{Payload Mass} = f(C_3)
                    \end{equation*}
                    \vspace{-0.2cm}
                    A required $C_3 = 15\text{ km}^2/\text{s}^2$ dictates immediately how many kilograms a Falcon Heavy or SLS can inject.
                \end{enumerate}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{The Reality of Interplanetary Flight: Beyond Hohmann}
    \begin{columns}[t]
        ...
        \begin{column}{0.47\textwidth}
            \begin{alertblock}{The Computational Solution: Lambert Sweeps}
                \scriptsize
                Instead of guessing transfer conics, astrodynamicists use numerical solvers to solve \textbf{Lambert's Problem} across thousands of calendar combinations:
                \vspace{-0.08cm}
                \begin{equation*}
                    (\mathbf{r}_1(t_{\text{dep}}), \, \mathbf{r}_2(t_{\text{arr}}), \, \Delta t) \xrightarrow{\text{Lambert}} (\mathbf{V}_1, \, \mathbf{V}_2)
                \end{equation*}
                \vspace{-0.15cm}
                Plotting the resulting $C_3$ values on a 2D date grid reveals the mission landscape: \textbf{The Porkchop Plot}!
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[t]
        ...
        \begin{column}{0.49\textwidth}
            \begin{block}{Porkchop Contour Schematic}
                \centering
                \begin{tikzpicture}[scale=0.64]
                    \draw[fill=DarkSlate, rounded corners=6pt] (-2.8,-2.0) rectangle (2.8, 2.0);
                    
                    \draw[->, >=stealth, color=white, line width=0.9pt] (-2.4,-1.6) -- (2.5,-1.6) node[right, font=\tiny]{$t_{\text{dep}}$};
                    \draw[->, >=stealth, color=white, line width=0.9pt] (-2.4,-1.6) -- (-2.4,1.8) node[above, font=\tiny]{$t_{\text{arr}}$};
                    
                    \draw[color=UdeAGold!80, line width=0.7pt] (-1.3,-0.2) ellipse (0.7cm and 1.1cm);
                    \draw[color=UdeAGreen!80, line width=0.9pt] (-1.3,-0.2) ellipse (0.45cm and 0.7cm);
                    \draw[color=cyan!80, line width=1pt] (-1.3,-0.2) ellipse (0.22cm and 0.35cm);
                    \node[color=cyan, font=\tiny] at (-1.3,-0.2) {$\bigstar$};
                    \node[color=white, font=\tiny] at (-1.3, 1.2) {Type I};
                    
                    \draw[color=UdeAGold!80, line width=0.7pt] (0.9, 0.4) ellipse (0.8cm and 1.0cm);
                    \draw[color=UdeAGreen!80, line width=0.9pt] (0.9, 0.4) ellipse (0.5cm and 0.65cm);
                    \draw[color=cyan!80, line width=1pt] (0.9, 0.4) ellipse (0.25cm and 0.35cm);
                    \node[color=cyan, font=\tiny] at (0.9, 0.4) {$\bigstar$};
                    \node[color=white, font=\tiny] at (0.9, 1.6) {Type II};
                    
                    \draw[dashed, color=AlertRed!80, line width=0.8pt] (-2.2, 0.2) -- (1.5, -1.5) node[midway, above right, sloped, font=\tiny]{$TOF = 200\text{ d}$};
                \end{tikzpicture}
            \end{block}
        \end{column}
    \end{columns}
    ...
    \begin{columns}[t]
        \begin{column}{0.50\textwidth}
            \begin{block}{Steps 1 to 3: The Lambert Sweep Loop}
                \scriptsize
                \begin{enumerate}\setlength{\itemsep}{1pt}\setlength{\topsep}{1pt}
                    \item \textbf{Define Constraints:} Select bodies, calendar windows ($t_{\text{dep}}, t_{\text{arr}}$), and launcher $C_3$ limits.
                    \item \textbf{Initialize 2D Grid:} Set calendar step size ($\Delta t = 2\text{ days}$) across both axes.
                    \item \textbf{Execute Lambert Sweep (Each point):}
                    \begin{itemize}\setlength{\itemsep}{0.5pt}
                        \item Query ephemerides $\mathbf{r}_1(t_{\text{dep}}), \mathbf{r}_2(t_{\text{arr}})$ from SPICE.
                        \item Solve Lambert BVP for $TOF = t_{\text{arr}} - t_{\text{dep}}$ to obtain $\mathbf{V}_1, \mathbf{V}_2$.
                        \item Excess vector: $\mathbf{v}_\infty = \mathbf{V}_1 - \mathbf{V}_{\text{planet},1}$.
                        \item Store parameter: $C_3 = \|\mathbf{v}_\infty\|^2$.
                    \end{itemize}
                \end{enumerate}
            \end{block}
        \end{column}
        ...
        \begin{column}{0.47\textwidth}
            \begin{block}{Steps 4 and 5: Window Selection}
                \scriptsize
                \begin{enumerate}\setcounter{enumi}{3}\setlength{\itemsep}{1.5pt}\setlength{\topsep}{1pt}
                    ...
                    \begin{itemize}\setlength{\itemsep}{1pt}
                        \item Locate local minima (valleys) in the contours.
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Type I Trajectories ($\Delta\nu < 180^\circ$)}
                ...
                \begin{itemize}\setlength{\itemsep}{1.5pt}
                    ...
                    \item \textbf{Shorter Time of Flight:} Spacecraft reaches target before aphelion (typically $\sim 200\text{ days}$ for Mars).
    ...
    \begin{columns}[t]
        \begin{column}{0.54\textwidth}
            \begin{block}{Keplerian Boundary Value Formulation}
                \scriptsize
                Determine the transfer orbit connecting planetary positions $\mathbf{r}_1$ and $\mathbf{r}_2$ in gravitational parameter $\mu$ over time of flight $\Delta t = t_2 - t_1$[cite: 1]:
                \vspace{-0.08cm}
                \begin{equation}
                    \Delta t = f(a, \|\mathbf{r}_1\| + \|\mathbf{r}_2\|, c)
                \end{equation}
                \vspace{-0.15cm}
                where $c = \|\mathbf{r}_2 - \mathbf{r}_1\|$ is the geometric chord[cite: 1].
            \end{block}
            \vspace{-0.1cm}
            \begin{block}{Izzo's Algorithm in \texttt{poliastro.iod.izzo}}
                \scriptsize
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item Parameterizes transfer geometry using transformed chord variables $(x, y)$[cite: 1].
                    \item Solves for semi-major axis $a$ using rapid Householder iterations[cite: 1].
                    \item Computes departure velocity $\mathbf{v}_1$ and arrival velocity $\mathbf{v}_2$[cite: 1].
                    \item Multi-revolution indexing: selects the 0-revolution direct solution (\texttt{sol[0]})[cite: 1].
                \end{itemize}
            \end{block}
        \end{column}
    ...
    \begin{columns}[t]
        \begin{column}{0.54\textwidth}
            \begin{block}{NASA SPICE Kernels Ingestion}
                \scriptsize
                High-fidelity interplanetary trajectory design requires high-precision ephemerides rather than mean Keplerian elements[cite: 1]:
                \begin{itemize}\setlength{\itemsep}{1.5pt}
                    \item \textbf{SPK (\texttt{de442.bsp}):} JPL ephemeris providing 3D states[cite: 1].
                    \item \textbf{LSK (\texttt{naif0012.tls}):} Leapseconds; UTC $\to$ Ephemeris Time[cite: 1].
                    \item \textbf{PCK (\texttt{gm\_de440.tpc}):} Planetary gravitational constants ($\mu$)[cite: 1].
                \end{itemize}
            \end{block}
            \vspace{-0.1cm}
            \begin{exampleblock}{Automated Kernel Ingestion}
                \scriptsize
                Kernels are downloaded via \texttt{urllib} and registered into memory via \texttt{spy.furnsh()}[cite: 1].
            \end{exampleblock}
        \end{column}
...
_, sun_values = spy.bodvrd('SUN', 'GM', 1)
mu_sun = sun_values[0] # km^3/s^2

# 3. Convert Calendar Dates to ET Seconds
    ...
    \begin{columns}[t]
        \begin{column}{0.54\textwidth}
            \begin{block}{The Multi-Planet Dimension Dilemma}
                ...
                \vspace{-0.08cm}
                \begin{equation*}
                    \mathbf{T} = [t_{\text{Earth}}, \; t_{\text{Jupiter}}, \; t_{\text{Saturn}}]^T \in \mathbb{R}^3 \quad \text{[cite: 1]}
                \end{equation*}
                \vspace{-0.15cm}
                A brute-force 3D grid ($100^3$ points) requires \textbf{$10^6$ double-Lambert evaluations}[cite: 1].
            \end{block}
            \begin{alertblock}{The Code Strategy: Freezing Jupiter--Saturn TOF}
                \scriptsize
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item Slices the 3D tensor into a 2D Opportunity Map[cite: 1]:
                    \vspace{-0.05cm}
                    \begin{equation*}
                        t_{\text{Saturn}} = t_{\text{Jupiter}} + \Delta t_{\text{fixed}} \quad (\Delta t_{\text{fixed}} = 2.0\text{ years}) \quad \text{[cite: 1]}
                    \end{equation*}
                    \vspace{-0.15cm}
                    \item Non-physical bounds ($t_{\text{Jupiter}} \le t_{\text{Earth}}$) are skipped with \texttt{continue}, leaving matrix entries as \texttt{np.nan}[cite: 1].
                \end{itemize}
            \end{alertblock}
        \end{column}
        ...
        \begin{column}{0.44\textwidth}
\begin{lstlisting}[style=pythoncode]
# Freeze Jupiter-Saturn cruise time (2 Years)
fixed_jup_sat_tof_sec = 2 * 365 * 86400
...
flyby_start = spy.utc2et("1979-01-01T00:00:00")
flyby_end   = spy.utc2et("1981-01-01T00:00:00")
    ...
    \begin{columns}[t]
        \begin{column}{0.50\textwidth}
            \begin{block}{Flyby Conservation vs. $\Delta v_{\text{penalty}}$}
                \scriptsize
                Inside the planetary SOI, mechanical energy is conserved without propulsion: $\|\mathbf{v}_{\infty,\text{in}}\| = \|\mathbf{v}_{\infty,\text{out}}\|$[cite: 1].
                \begin{itemize}\setlength{\itemsep}{1pt}
                    \item \textbf{Leg 1 (Earth $\to$ Jupiter):} Lambert yields $\mathbf{v}_{1,J}$[cite: 1]. Subtracting Jupiter's heliocentric velocity $\mathbf{V}_J$ gives $\mathbf{v}_{\infty,\text{in}} = \mathbf{v}_{1,J} - \mathbf{V}_J$[cite: 1].
                    \item \textbf{Leg 2 (Jupiter $\to$ Saturn):} Lambert yields $\mathbf{v}_{2,J}$[cite: 1]. Outbound excess: $\mathbf{v}_{\infty,\text{out}} = \mathbf{v}_{2,J} - \mathbf{V}_J$[cite: 1].
                    \item \textbf{Matching Penalty Function:}
                    \vspace{-0.05cm}
                    \begin{equation*}
                        \Delta v_{\text{penalty}} = \big| \|\mathbf{v}_{\infty,\text{out}}\| - \|\mathbf{v}_{\infty,\text{in}}\| \big| \quad \text{[cite: 1]}
                    \end{equation*}
                    \vspace{-0.15cm}
                    A ballistic flyby occurs when $\Delta v_{\text{penalty}} \to 0$![cite: 1]
                \end{itemize}
            \end{block}
        \end{column}
...
dv_penalty_grid[i, j] = abs(np.linalg.norm(v_inf_out) - np.linalg.norm(v_inf_in))
\end{lstlisting}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[t]
        \begin{column}{0.50\textwidth}
            \begin{block}{Unraveling 2D Matrix Minimum}
                \scriptsize
                \begin{itemize}\setlength{\itemsep}{1.5pt}
                    \item \texttt{np.nanargmin()} returns the flat 1D index of the global minimum penalty[cite: 1].
                    \item \texttt{np.unravel\_index()} restores the 2D matrix indices $(i_{\text{opt}}, j_{\text{opt}})$ corresponding to optimal epochs[cite: 1].
                    \item \texttt{spy.et2utc()} converts Ephemeris Times to human-readable UTC strings[cite: 1].
                    \item \texttt{plt.contourf()} renders filled penalty isochrones, overlaying a star marker at the global optimum[cite: 1].
                \end{itemize}
            \end{block}
            \vspace{-0.1cm}
            \begin{exampleblock}{The 1977 Voyager Outcome}
                \scriptsize
                The algorithm automatically pinpoints the historic Voyager 2 launch corridor on \textbf{August 22, 1977} ($\Delta v_{\text{penalty}} \approx 0.08\text{ km/s}$)![cite: 1]
            \end{exampleblock}
        \end{column}
...
best_earth_utc = spy.et2utc(earth_dates[min_j], 'C', 0)
best_jup_utc   = spy.et2utc(jup_dates[min_i], 'C', 0)
...
plt.plot(earth_dt[min_j], jup_dt[min_i], marker='*', color='red', markersize=14)
\end{lstlisting}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[t]
        \begin{column}{0.50\textwidth}
            \begin{block}{Heliocentric Cruise Mechanics}
                ...
                \vspace{-0.08cm}
                \begin{equation*}
                    \mathbf{V}_{\text{tx},1} = \mathbf{V}_{\text{planet},1} + \mathbf{v}_{\infty,\text{dep}} \quad \text{[cite: 2]}
                \end{equation*}
                \vspace{-0.15cm}
                \begin{itemize}\setlength{\itemsep}{1.5pt}
                    \item \textbf{Prograde $\mathbf{v}_\infty$:} Raises aphelion ($R_a > R_1$) toward outer planets (Mars, Jupiter)[cite: 2].
                ...
                \vspace{-0.08cm}
                \begin{equation*}
                    \mathbf{v}_{\infty,\text{arr}} = \mathbf{V}_{\text{tx},2} - \mathbf{V}_{\text{planet},2} \quad \text{[cite: 2]}
                \end{equation*}
            \end{block}
        \end{column}
        ...
        \begin{column}{0.47\textwidth}
            \begin{block}{Phase 2 Heliocentric Transfer Arc}
                ...
                \begin{tikzpicture}[scale=0.64]
                    ...
                    \node[color=blue!80, font=\tiny] at (-1.5, 0.9) {Orbit 1 (Earth)};
                    
                    \draw[thick, color=orange!80] (0,0) circle (2.3cm);
                    \node[color=orange!90, font=\tiny] at (1.65, 2.05) {Orbit 2 (Target)};
        ...
        \begin{column}{0.50\textwidth}
            \begin{block}{Arrival Hyperbola and Orbit Insertion}
                ...
                \begin{enumerate}\setlength{\itemsep}{1.5pt}\setlength{\topsep}{1pt}
                    ...
                    \item Periapsis speed at closest approach $r_{p,2}$[cite: 2]:
                    \vspace{-0.08cm}
                    \begin{equation*}
                        v_{p,\text{hyp}} = \sqrt{v_{\infty,\text{arr}}^2 + \frac{2\mu_2}{r_{p,2}}} \quad \text{[cite: 2]}
                    \end{equation*}
                    \vspace{-0.15cm}
                    \item \textbf{Orbit Insertion Burn ($\Delta v_{\text{cap}}$):} Retrograde burn at periapsis captures probe into circular orbit ($v_c = \sqrt{\mu_2/r_{p,2}}$)[cite: 2]:
                    \vspace{-0.08cm}
                    \begin{equation*}
                        \Delta v_{\text{cap}} = \sqrt{v_{\infty,\text{arr}}^2 + \frac{2\mu_2}{r_{p,2}}} - \sqrt{\frac{\mu_2}{r_{p,2}}} \quad \text{[cite: 2]}
                    \end{equation*}
                \end{enumerate}
            \end{block}
        \end{column}
        ...
        \begin{column}{0.47\textwidth}
            \begin{block}{Phase 3 Vector Geometry}
                ...
                \begin{tikzpicture}[scale=0.64]
                    ...
                    \draw[thick, ->, >=stealth, color=Charcoal] (0,0) -- (0,1.25) node[left, font=\tiny]{$\mathbf{v}_{\text{planet}}$};
                    
                    \draw[thick, color=UdeAGold, line width=0.9pt] (0,0) ellipse (0.75cm and 0.55cm);
                    \node[color=UdeAGold!90!black, font=\tiny] at (-1.2, -0.35) {Captured Orbit};
                    ...
                    \node[above right, color=AlertRed, font=\tiny] at (0, 0.55) {$\Delta v_{\text{cap}}(r_p)$};
        ...
        \begin{column}{0.47\textwidth}
            \begin{block}{Flyby Vector Geometry}
                ...
                \begin{tikzpicture}[scale=0.68]
                    \draw[thick, fill=gray!25] (0,0) circle (0.38cm);
                    \node[below=0.42cm, font=\tiny] at (0,0) {Planet ($\mu_p$)};
...
% ==============================================================================
% SLIDE 33: IMPLEMENTATION IN NASA GMAT
% ==============================================================================
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{GMAT Targeter Architecture}
                ...
                \begin{itemize}\setlength{\itemsep}{1pt}
                    ...
                    \item \textbf{Achieve Commands (Goals):} Constrain target parameters at Mars:
                    \begin{itemize}\setlength{\itemsep}{0pt}
                        \item \texttt{Achieve DC(MarsProbe.MarsInertial.BdotT = -4500)}
                        \item \texttt{Achieve DC(MarsProbe.MarsInertial.BdotR = +2800)}
                    \end{itemize}
                \end{itemize}
            \end{block}
            ...
            \begin{alertblock}{Forensic Check: Frame Origin}
                \scriptsize
                $B\cdot T$ and $B\cdot R$ must be evaluated in \textbf{Mars Centered Frame} (\texttt{MarsInertial}), never in \texttt{EarthMJ2000}!
            \end{alertblock}
        \end{column}
...
Target 'Target Mars B-Plane' DC {SolveMode = Solve}
    Vary 'TCM_Vx' DC(TCM.Element1 = 0.05, {MaxStep = 0.01})
    Vary 'TCM_Vy' DC(TCM.Element2 = -0.02, {MaxStep = 0.01})
    Vary 'TCM_Vz' DC(TCM.Element3 = 0.01, {MaxStep = 0.01})
    ...
    Achieve 'Target BdotT' ...
         DC(MarsProbe.MarsInertial.BdotT = -4500.0, {Tol = 0.1});
    Achieve 'Target BdotR' ...
         DC(MarsProbe.MarsInertial.BdotR = 2800.0, {Tol = 0.1});
EndSubSequence;
\end{lstlisting}
        \end{column}
    \end{columns}
