...
\lstdefinestyle{gmatstyle}{
    ...
    basicstyle=\ttfamily\fontsize{4.8pt}{5.5pt}\selectfont\color{Charcoal},
...
\lstdefinestyle{pythoncode}{
    ...
    basicstyle=\ttfamily\fontsize{4.6pt}{5.3pt}\selectfont\color{Charcoal},
    keywordstyle=\bfseries\color{UdeAGreen},
    commentstyle=\itshape\color{gray},
    stringstyle=\color{UdeAGold!90!black},
...
earth_dt = [datetime.datetime.strptime(
    spy.et2utc(t, 'ISOC', 0), "%Y-%m-%dT%H:%M:%S") 
    for t in earth_dates]
...
% ==============================================================================
% SLIDE: NUMERICAL BOUNDARY SOLVERS & THE DIFFERENTIAL CORRECTOR
% ==============================================================================
    ...
    \begin{columns}[t]
        \begin{column}{0.53\textwidth}
            \begin{block}{Two-Point Boundary Value Problem (TPBVP)}
                ...
                \begin{itemize}\setlength{\itemsep}{1pt}
                    ...
                    \item \textbf{Goal Vector ($\mathbf{Y} \in \mathbb{R}^m$):} Constrained arrival states (e.g., $B\cdot T, B\cdot R$, altitude $r_p$).
                \end{itemize}
            \end{block}
            \vspace{-0.1cm}
            \begin{block}{Newton-Raphson Shooting & The Jacobian ($\mathbf{J}$)}
                \scriptsize
                The Differential Corrector (DC) linearizes state transitions around the current trajectory via the sensitivity Jacobian matrix:
                \vspace{-0.05cm}
                \begin{equation*}
        ...
        \begin{column}{0.44\textwidth}
            \begin{block}{The Forward Shooting Mechanism}
                ...
                \begin{tikzpicture}[scale=0.68]
                    % Departure Node
                    \filldraw[color=SpiceBlue] (-2.2, -1.0) circle (3pt) node[below left, font=\tiny]{Earth ($t_0$)};
                    
                    % Target Planet & Goal
                    \draw[dashed, color=gray] (1.8, 1.2) circle (0.85cm);
                    \filldraw[color=orange!80] (1.8, 1.2) circle (4pt) node[below right, font=\tiny]{Target};
                    \filldraw[color=UdeAGreen] (1.2, 1.4) circle (2.5pt) node[above left, font=\tiny]{$\mathbf{Y}^*$ (Goal)};
                    
                    % Trajectory 0 (Nominal Miss)
                    \draw[thick, color=AlertRed, dashed] (-2.2, -1.0) to[out=25, in=-120] (0.0, 0.4) to[out=60, in=-140] (1.9, -0.2);
                    \filldraw[color=AlertRed] (1.9, -0.2) circle (2pt) node[right, font=\tiny]{$\mathbf{Y}(\mathbf{X}_0)$ [Miss]};
                    
                    % Perturbed Trajectory
                    \draw[thick, color=UdeAGold, dotted] (-2.2, -1.0) to[out=32, in=-115] (0.1, 0.7) to[out=65, in=-135] (1.7, 0.5);
                    \filldraw[color=UdeAGold!90!black] (1.7, 0.5) circle (1.8pt) node[right, font=\tiny]{Perturbed};
                    
                    % Converged Trajectory
                    \draw[thick, color=UdeAGreen, line width=1.2pt] (-2.2, -1.0) to[out=38, in=-110] (0.0, 0.9) to[out=70, in=-120] (1.2, 1.4);
                    \node[color=UdeAGreen, font=\tiny\bfseries] at (-0.7, 0.8) {Converged ($\mathbf{X}^*$)};
                \end{tikzpicture}
            \end{block}
            \vspace{-0.1cm}
            \begin{alertblock}{Iterative Convergence}
                \scriptsize Execution loops until residual error $\|\mathbf{Y}(\mathbf{X}_k) - \mathbf{Y}^*\| \le \boldsymbol{\epsilon}_{\text{tol}}$.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
...
% ==============================================================================
% SLIDE: GMAT TARGET COMMAND & MISSION SEQUENCE ARCHITECTURE
% ==============================================================================
    ...
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            \begin{block}{The Four Pillars of the Target Loop}
                \scriptsize
                GMAT structures the shooting solver into an isolated execution block via four specialized commands:
                \begin{enumerate}\setlength{\itemsep}{2pt}\setlength{\topsep}{1pt}
                    ...
                    \item \textbf{Dynamic Propagation:} Applies the maneuver (\texttt{Maneuver}) and propagates the spacecraft forward (\texttt{Propagate}) to a terminal event condition.
                    \item \textbf{\texttt{Achieve} (The Goals):} Asserts boundary constraints $\mathbf{Y}^*$ evaluated at the stopping condition, along with acceptable tolerances.
                \end{enumerate}
            \end{block}
        \end{column}
...
% Define Targeter Loop using Differential Corrector
Target 'Mars B-Plane Targeting' DC {SolveMode = Solve}

    % 1. VARY COMMANDS (Independent Variables X)
    Vary 'Vary TCM_Vx' DC(TCM.Element1 = 0.05, ...
         {Perturbation = 1e-4, Lower = -1.0, Upper = 1.0, MaxStep = 0.02})
    Vary 'Vary TCM_Vy' DC(TCM.Element2 = -0.02, ...
         {Perturbation = 1e-4, Lower = -1.0, Upper = 1.0, MaxStep = 0.02})
    Vary 'Vary TCM_Vz' DC(TCM.Element3 = 0.01, ...
         {Perturbation = 1e-4, Lower = -1.0, Upper = 1.0, MaxStep = 0.02})

    % 2. STATE PROPAGATION (Forward Physics)
    Maneuver 'Apply TCM' TCM(MarsProbe);
    Propagate 'Cruise to Mars Periapsis' DeepSpaceProp(MarsProbe) ...
         {MarsProbe.Mars.Periapsis};

    % 3. ACHIEVE COMMANDS (Dependent Constraints Y)
    Achieve 'Target BdotT' ...
         DC(MarsProbe.MarsInertial.BdotT = -4500.0, {Tolerance = 0.5});
    Achieve 'Target BdotR' ...
         DC(MarsProbe.MarsInertial.BdotR =  2800.0, {Tolerance = 0.5});

EndTarget;  % Concludes SubSequence Convergence
\end{lstlisting}
        \end{column}
    \end{columns}
\end{frame}
...
% ==============================================================================
% SLIDE: FORENSIC DC CONVERGENCE & TUNING RULES
% ==============================================================================
