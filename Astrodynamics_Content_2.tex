...
\subtitle{Forensic Numerical Engineering \& Constrained Interplanetary Flight Mechanics}
...
% ==============================================================================
% SLIDE 1: ROADMAP (RESIZED & BALANCED)
% ==============================================================================
\begin{frame}{Module 2: Architecture \& Operational Roadmap}
    \begin{columns}[t]
        \begin{column}{0.60\textwidth}
            \begin{block}{From Planar Transfers to Constrained 3D Navigation}
                \footnotesize
                \begin{itemize}
                    \item \textbf{Week 5: 3D Patched Conics \& Hyperbolic Asymptotes}
                    \begin{itemize}
                        \item Expanding 2D Lambert transfers to 3D inertial space.
                        \item Asymptote coordinates ($\alpha, \delta$) and the \textbf{Inclination Trap} ($i \ge |\delta|$).
                    \end{itemize}
                    \item \textbf{Week 6: Gravity Assist Mechanics \& Powered Flybys}
                    \begin{itemize}
                        \item Planetary SOI momentum exchange; turning angle $\delta_{\text{turn}}(r_p)$.
                        \item Leading vs. Trailing encounters and deep-space Oberth burns.
                    \end{itemize}
                    \item \textbf{Week 7: The B-Plane Targeting Framework}
                    \begin{itemize}
                        \item $(\hat{\mathbf{S}}, \hat{\mathbf{T}}, \hat{\mathbf{R}})$ frame derivation and impact parameter $\mathbf{B}$.
        ...
        \begin{column}{0.37\textwidth}
            ...
            \begin{tikzpicture}[scale=0.55]
                \draw[thick, ->, >=stealth, color=UdeAGreen] (0,0) -- (2.6,0) node[right]{\scriptsize $\hat{\mathbf{T}}$};
                \draw[thick, ->, >=stealth, color=UdeAGreen] (0,0) -- (0,-2.0) node[below]{\scriptsize $\hat{\mathbf{R}}$};
                \draw[thick, ->, >=stealth, color=UdeAGold] (0,0) -- (1.9,-1.3) node[midway, above right]{\scriptsize $\mathbf{B}$};
                \draw[dashed, color=gray] (1.9,-1.3) -- (1.9,0);
                \draw[dashed, color=gray] (1.9,-1.3) -- (0,-1.3);
...
\begin{frame}{Worked Example 1: Earth-to-Mars Departure Geometry}
    ...
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            ...
            \end{align*}
            \textbf{Step 2: Asymptote Declination \& RA}
            \begin{equation*}
...
% ==============================================================================
\begin{frame}{Heliocentric Momentum Transfer \& Powered Flybys}
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
...
\begin{frame}{Worked Example 2: Voyager 2 Jupiter Gravity Assist}
    ...
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            \footnotesize
            \textbf{Step 1: Eccentricity \& Turning Angle}
            \begin{align*}
    ...
    \begin{columns}[t]
        ...
        \begin{column}{0.43\textwidth}
            \begin{block}{B-Plane Geometry}
                \centering
                \begin{tikzpicture}[scale=0.65]
                    \draw[thick, fill=LightBg] (0,0) circle (1.3cm);
                    \draw[thick, ->, >=stealth, color=Charcoal] (-1.3,0) -- (1.3,0) node[right]{\tiny $\hat{\mathbf{T}}$};
                    \draw[thick, ->, >=stealth, color=Charcoal] (0,1.3) -- (0,-1.3) node[below]{\tiny $\hat{\mathbf{R}}$};
                    \filldraw[color=UdeAGreen] (0,0) circle (2pt) node[below left]{\tiny Target};
                    \draw[thick, ->, >=stealth, color=AlertRed, line width=0.8pt] (0,0) -- (0.9,-0.7) node[midway, above right]{\tiny $\mathbf{B}$};
                    \draw[dashed, color=gray] (0.9,-0.7) -- (0.9,0) node[above]{\tiny $B\cdot T$};
                    \draw[dashed, color=gray] (0.9,-0.7) -- (0,-0.7) node[left]{\tiny $B\cdot R$};
                \end{tikzpicture}
            \end{block}
        \end{column}
    \end{columns}
...
% ==============================================================================
\begin{frame}{B-Plane Target Quantities \& Impact Parameter Derivations}
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
...
% ==============================================================================
% SLIDE 10: NASA GMAT SCRIPTING (FRAGILE FRAME)
% ==============================================================================
    ...
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            \begin{block}{GMAT Targeter Architecture}
                \footnotesize
                \begin{itemize}
                    \item \textbf{Vary Commands:} Adjust TCM velocity components ($\Delta V_X, \Delta V_Y, \Delta V_Z$).
                    \item \textbf{Achieve Commands:} Target arrival coordinates at Mars:
                    \begin{itemize}
                        \item \texttt{Achieve DC(Sat.MarsInertial.BdotT = -4500)}
                        \item \texttt{Achieve DC(Sat.MarsInertial.BdotR = +2800)}
                    \end{itemize}
                \end{itemize}
            \end{block}
            \begin{alertblock}{Forensic Check: Coordinate Origin}
                \scriptsize
                $B\cdot T$ and $B\cdot R$ must be evaluated in \textbf{Mars Centered Frame} (\texttt{MarsInertial}), never EarthMJ2000!
            \end{alertblock}
        \end{column}
...
Target 'Target Mars B-Plane' DC {SolveMode = Solve}
    Vary 'Vary TCM_Vx' DC(TCM.Element1 = 0.05, ...
         {Perturbation = 1e-4, MaxStep = 0.01})
    ...
    Propagate 'To Mars Periapsis' ...
         DeepSpaceProp(MarsProbe) ...
...
% ==============================================================================
% SLIDE 11: FORENSIC BUG AUDIT
% ==============================================================================
\begin{frame}{Forensic Bug Audit: Identifying Non-Linear Navigation Errors}
    \begin{columns}[t]
        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Case A: The "Inclination Violation"}
                \scriptsize
                \textbf{Observed Defect:}
                GMAT targeter fails to converge on a Mars departure arc from LEO ($i = 28.5^\circ$), oscillating infinitely.
                
                \textbf{Root Cause Analysis:}
                Lambert solver computed $\delta = +36.2^\circ$. Because $i < |\delta|$, no coplanar injection exists, causing Jacobian singularity in differential corrector.
            \end{alertblock}
        \end{column}

        \begin{column}{0.48\textwidth}
            \begin{alertblock}{Case B: B-Plane Sign Reversal}
                \scriptsize
                \textbf{Observed Defect:}
                Aiming for a trailing gravity assist at Jupiter, spacecraft passes leading side, losing $8\text{ km/s}$.

                \textbf{Root Cause Analysis:}
                Angle $\theta_B$ was set to $+45^\circ$ instead of $-135^\circ$, projecting $\mathbf{B}$ in front of Jupiter's orbital path and flipping energy gain to loss.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
...
% ==============================================================================
\begin{frame}{Module 2 Synthesis \& Midterm Take-Home Challenge}
    \begin{columns}[t]
        \begin{column}{0.50\textwidth}
